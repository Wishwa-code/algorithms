...
void add_pairs(void)
{
    ...
    pair_count = index+1; // this needs to be reduced by 1 according isntructions

    for (int k = 0; k<index; k++)
    {
        printf("added_pairs %i / %i -> %i %i %i\n",k,pair_count-1,pairs[k].winner, pairs[k].loser, preferences[pairs[k].winner][pairs[k].loser]);
    }
    return;
}
...
void sort_pairs(void)
{
    ...
    {
        printf("soreted pair %i out of %i %i %i %i\n", k, pair_count-1, pairs[k].winner, pairs[k].loser, preferences[pairs[k].winner][pairs[k].loser]);
    }
